...
execute if entity @e[type=marker,tag=RingOfFire,predicate=cannons:ships/tutorialfull] unless entity @e[type=marker,tag=RingOfFire,limit=1] run gamerule minecraft:fire_spread_radius_around_player 0
execute if entity @e[type=marker,tag=RingOfFire,predicate=cannons:ships/tutorialfull] unless score $gamestate CmdData matches 2 run gamerule minecraft:fire_spread_radius_around_player 0
