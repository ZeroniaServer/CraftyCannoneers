...
#> Pirate Teachers
execute as @e[type=wandering_trader,tag=Teacher] run data merge entity @s {Offers:{Recipes:[]},CustomName:'["",{"translate":"space.-4096"},{"text":"\\uE003"},{"translate":"space.-4096"}]',HandItems:[{},{}],NoAI:1b,Silent:1b}
...
execute as @e[type=wandering_trader,tag=CombatTeacher] at @s unless entity @a[team=Lobby,distance=..10,limit=1] run rotate @s 0 0
execute as @e[type=wandering_trader,tag=WeakpointTeacher] at @s unless entity @a[team=Lobby,distance=..10,limit=1] run rotate @s 180 0