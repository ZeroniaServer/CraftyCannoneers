...
execute unless score @s hasArrows > @s arrowsToShoot store result storage craftycannoneers:chest Items[0].Count byte 0.99999 run data get storage craftycannoneers:chest Items[0].Count
execute unless score @s[team=Orange] hasArrows > @s arrowsToShoot run data modify entity @e[type=minecraft:chest_minecart,tag=orangeteamchest,distance=..7,limit=1] Items append from storage craftycannoneers:chest Items[0]
execute unless score @s[team=Purple] hasArrows > @s arrowsToShoot run data modify entity @e[type=minecraft:chest_minecart,tag=purpleteamchest,distance=..7,limit=1] Items append from storage craftycannoneers:chest Items[0]
