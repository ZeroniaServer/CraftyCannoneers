...
execute at @s if entity @a[gamemode=!spectator,distance=..6] run function cannons:bounce/tpslime
execute at @s unless score $tp bounceID matches 1 if entity @a[gamemode=!spectator,distance=..6] run function cannons:bounce/summonslime
