...
execute unless score $OrangeWPDelay CmdData matches 1.. if entity @s[tag=HitSpotted,tag=!WeakpointAnnounced] as @a[team=!Lobby,team=!Spectator,team=!Developer] if score @s playerUUID = $tempuuid playerUUID run tellraw @a[team=Orange] ["","\n",{"text":"Enemy Weakpoint destroyed!","italic":true,"color":"green"},"\n",{"selector":"@s"},{"text":" destroyed an ","color":"dark_aqua"},{"text":"exposed","underlined":true,"color":"dark_green"},{"text":" Weakpoint.","color":"dark_aqua"},"\n"]
execute unless score $OrangeWPDelay CmdData matches 1.. if entity @s[tag=HitSpotted,tag=!WeakpointAnnounced] as @a[team=!Lobby,team=!Spectator,team=!Developer] if score @s playerUUID = $tempuuid playerUUID run tellraw @a[team=Purple] ["","\n",{"selector":"@s"},{"text":" destroyed one of your ship's Weakpoints!","color":"red"},"\n"]
