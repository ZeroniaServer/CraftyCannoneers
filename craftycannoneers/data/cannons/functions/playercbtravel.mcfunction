...
execute as @a[tag=PlayerCannon] at @s run loot replace entity @e[type=armor_stand,tag=PCannonballdeco,tag=!hashead,limit=1,sort=nearest] armor.head loot cannons:playerhead
execute as @a[tag=PlayerCannon] at @s run tag @e[type=armor_stand,tag=PCannonballdeco,tag=!hashead,limit=1,sort=nearest] add hashead
