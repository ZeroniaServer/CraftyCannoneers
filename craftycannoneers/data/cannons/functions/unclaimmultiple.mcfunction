...
execute as @e[type=armor_stand,tag=CannonDisp,tag=!FireCannon] if score @s playerUUID = $hascannon playerUUID run scoreboard players add $cannonamount playerUUID 1
...
execute if score $cannonamount playerUUID matches 2.. as @e[type=armor_stand,tag=CannonDisp,tag=!NewClaim] if score @s playerUUID = $hascannon playerUUID run tag @s remove CurrentCheck
