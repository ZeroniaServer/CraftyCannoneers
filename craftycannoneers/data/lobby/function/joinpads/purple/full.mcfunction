...
execute unless score @s joinpurple matches 1.. unless score $InOrange CmdData >= $MaxTeamSize CmdData run title @s subtitle ["",{"translate":"game.try_join","color":"dark_aqua","bold":false,"with":[{"translate":"game.try_join.orange","color":"gold","bold":true},{"translate":"game.try_join.team","color":"dark_aqua","bold":false}]}]
