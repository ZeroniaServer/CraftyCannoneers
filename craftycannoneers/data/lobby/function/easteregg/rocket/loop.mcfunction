...
execute at @s[scores={CmdData=102}] run summon area_effect_cloud -57 -18 -21 {Tags:["RRText"],Duration:80,CustomNameVisible:1b,CustomName:["",{translate:"easter_egg.launch.should_try",color:"white",with:[{translate:"easter_egg.launch.rocket",bold:true,color:"blue"},{translate:"easter_egg.launch.riders",bold:true,color:"gold"}]}],Radius:0f,Particle:{type:"block",block_state:"minecraft:air"}}
