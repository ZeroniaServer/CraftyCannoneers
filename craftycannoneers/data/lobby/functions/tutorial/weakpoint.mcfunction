...
execute if entity @s[scores={WeakpointTutorial=6}] run tellraw @s ["",{"text":"[6/6] ","color":"gray"},{"text":"Usually you can only see 1 Weakpoint at a time, but here in this Watchtower, you will be able to see multiple of them, just so you can properly see what they look like.","color":"green"},"\n"]
execute if entity @s[scores={WeakpointTutorial=5}] run tellraw @s ["",{"text":"[5/6] ","color":"gray"},{"text":"Hitting Exposed Ship Weakpoints using any type Cannonball will deal a lot of additional damage, so make sure to spot those Weakpoints and aim your shots to try and hit them.","color":"green"},"\n"]
execute if entity @s[scores={WeakpointTutorial=4}] run tellraw @s ["",{"text":"[4/6] ","color":"gray"},{"text":"Once a Weakpoint is exposed, it will temporarily be visible for your entire team, whether they use their Spotting Spyglass or not. The X shape will change to a red color and fade out over time, indicating that it's becoming unexposed.","color":"green"},"\n"]
