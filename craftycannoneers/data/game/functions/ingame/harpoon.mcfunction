...
execute if score $harpoon CmdData matches 0..1 unless score @s harpoonstate = @s harpoonstate2 run function game:harpoon/replace
scoreboard players operation @s harpoonstate2 = @s harpoonstate

#> When you've used an active harpoon, replace it with an inactive harpoon (no riptide and no durability)
execute at @s[predicate=game:harpoon/active_mainhand,scores={useharpoon=1..}] run loot replace entity @s weapon.mainhand loot weapons:harpoon_inactive
execute at @s[predicate=game:harpoon/active_offhand,scores={useharpoon=1..}] run loot replace entity @s weapon.offhand loot weapons:harpoon_inactive
