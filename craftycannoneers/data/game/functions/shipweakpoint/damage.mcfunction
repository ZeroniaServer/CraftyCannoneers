...
execute as @e[type=marker,tag=FakePointMarker,tag=!WeakpointDamaged] at @s positioned ~-5 ~ ~ if entity @e[type=marker,tag=ImpactMarker,distance=..5] run function game:shipweakpoint/fakerespawn