execute if entity @s[type=boat] run summon boat ~ ~ ~ {Rotation:[104.0f,0.0f],Invulnerable:1b,Tags:["Boat","BoatBoat","new","Rotate"],Passengers:[{id:"minecraft:interaction",Tags:["boatoccupant"],width:2,height:1,response:1b}]}

execute if entity @s[type=boat] as @e[type=boat,tag=new] at @s anchored feet rotated ~ 0 run summon armor_stand ^ ^-0.1 ^-0.6 {Rotation:[104.0f,0.0f],Tags:["Boat","BoatAS","new","Rotate"],Marker:1b,Invulnerable:1b,NoGravity:1b,Invisible:1b,ArmorItems:[{},{},{},{id:"minecraft:black_banner",Count:1b,tag:{BlockEntityTag:{Patterns:[{Pattern:sku,Color:0}]}}}]}

execute if entity @s[type=boat] if score $BoatCannons CmdData matches 1 as @e[type=boat,tag=new] at @s anchored feet rotated ~ 0 run summon camel ^ ^0.04 ^-0.4 {Age:-2147483648,Silent:1b,DeathTime:17s,Invulnerable:1b,Team:"NoName",PersistenceRequired:1b,NoAI:1b,Rotation:[104.0f,0.0f],Tags:["BoatCannonAnchor","new"]}
...
execute if entity @s[type=boat] if score $BoatCannons CmdData matches 1 as @e[type=villager,tag=new] at @s run summon item_display ~ ~0.73125 ~ {Rotation:[104.0f,0.0f],Tags:["BoatCannonDisplay","new"]}
...
execute at @s[type=villager,tag=BoatCannon] run summon husk ~ ~0.73125 ~ {Silent:1b,Invulnerable:1b,DeathTime:17s,DeathLootTable:"minecraft:empty",PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,IsBaby:1b,Attributes:[{Name:"generic.follow_range",Base:0},{Name:"generic.attack_damage",Base:0}],Rotation:[104.0f,0.0f],Team:"NoName",Tags:["BoatCannonDisplay","new"],ActiveEffects:[{Id:14,Amplifier:1b,Duration:-1,ShowParticles:0b}]}
