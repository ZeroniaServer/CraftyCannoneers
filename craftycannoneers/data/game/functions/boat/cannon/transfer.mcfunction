...
execute unless entity @s[tag=FireCannon] run function game:boat/cannon/updategunpowder
execute if entity @s[tag=FireCannon] on passengers on passengers on passengers run item replace entity @s[type=husk] armor.head with mooshroom_spawn_egg{CustomModelData:12}