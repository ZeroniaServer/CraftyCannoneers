...
execute rotated as @s[tag=Sailing,tag=!FireCannon,scores={PowerM=1..4}] rotated ~ ~40 on passengers if entity @s[type=player] anchored eyes positioned as @s anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.3] run scoreboard players set $rightdir CmdData 1
execute rotated as @s[tag=Sailing,tag=!FireCannon,scores={PowerM=5}] rotated ~ ~40 on passengers if entity @s[type=player,predicate=!cannons:holdgp] anchored eyes positioned as @s anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.3] run scoreboard players set $rightdir CmdData 1
execute at @s if entity @e[type=lightning_bolt,distance=..2] run scoreboard players set $rightdir CmdData 0

#> Detect occupied boat and adjust hitbox + replace display
execute if entity @s[tag=Sailing] on passengers on passengers on passengers if entity @s[type=item_display,tag=BoatCannonDisplay] on vehicle at @s run function game:boat/cannon/displayreplace

#> Prevent clicking on Cannon if not looking in right direction
execute if score $rightdir CmdData matches 0 if entity @s[tag=Sailing] on passengers if entity @s[type=camel,tag=!swapped] run function game:boat/cannon/swap
execute if score $rightdir CmdData matches 1 if entity @s[tag=Sailing] on passengers if entity @s[type=camel,tag=swapped] run function game:boat/cannon/swapback

#> Rotate the Cannon along with the boat
execute rotated as @s on passengers positioned as @s[type=camel] run tp @s ~ ~ ~ ~ ~
execute rotated as @s on passengers on passengers positioned as @s[type=villager] run tp @s ~ ~ ~ ~ ~
