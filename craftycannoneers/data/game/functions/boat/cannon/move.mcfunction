...
execute if entity @s[tag=Sailing] on passengers on passengers on passengers on passengers on passengers if entity @s[type=item_display,tag=BoatCannonDisplay] on vehicle at @s run function game:boat/cannon/displayreplace

#> Prevent clicking on Cannon if not looking in right direction
execute if score $rightdir CmdData matches 0 if entity @s[tag=Sailing] on passengers on passengers run data merge entity @s[type=interaction] {width:0,height:0,response:0b}
execute if score $rightdir CmdData matches 1 if entity @s[tag=Sailing] on passengers on passengers run data merge entity @s[type=interaction] {width:1,height:1,response:1b}

#> Rotate the Cannon along with the boat
execute rotated as @s on passengers positioned as @s[type=camel] run tp @s ~ ~ ~ ~ ~
execute rotated as @s on passengers on passengers on passengers on passengers on passengers positioned as @s run tp @s[tag=BoatCannonDisplay] ~ ~ ~ ~ ~
