...
execute as @a[tag=hitter,limit=1] run data modify block ~ 256 ~ front_text.messages[0] set value '["",{"translate":"cargobarrel.trap.using","with":[{"selector":"@a[limit=1,predicate=game:matches_uuid]"}]}]'
