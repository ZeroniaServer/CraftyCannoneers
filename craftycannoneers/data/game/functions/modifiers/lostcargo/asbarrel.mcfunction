...
#> Ascend
execute at @s[scores={CmdData=1..35}] run function game:modifiers/lostcargo/ascend
execute at @s[scores={CmdData=36..47}] run tp @s ~ ~-0.05 ~

#> Emerge
execute at @s[scores={CmdData=40}] run function game:modifiers/lostcargo/emerge

#> As hopper minecart
execute on passengers at @s[type=hopper_minecart] run function chests:aschests
