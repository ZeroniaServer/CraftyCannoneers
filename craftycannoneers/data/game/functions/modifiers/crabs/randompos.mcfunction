...
execute at @s if entity @s[tag=!Nope,tag=!SpawnNew] as @e[type=item_display,tag=CrabTrap,distance=..1] at @s run function weapons:crabtrap/break
execute at @s if entity @s[tag=!Nope,tag=!SpawnNew] unless entity @e[type=armor_stand,tag=chest,distance=..8] unless score $total crabs matches 5.. run function game:modifiers/crabs/spawn
