...
execute if score @s[tag=Instant,tag=InSafeZone] CmdData matches 30 run summon lightning_bolt ~ ~ ~
execute if score @s[tag=Instant,tag=!InSafeZone] CmdData matches 30 run summon lightning_bolt ~ ~ ~ {Tags:["NoDamage"]}
...
execute if score @s[tag=Instant,tag=!InSafezone,tag=HasUUID] CmdData matches 30 run scoreboard players operation @e[type=marker,tag=ImpactMarker,tag=lightningstrike,tag=!HasUUID] playerUUID = @s playerUUID
execute if score @s[tag=Instant,tag=!InSafezone,tag=HasUUID] CmdData matches 30 run data modify storage craftycannoneers:temp CustomName set from entity @s CustomName
execute if score @s[tag=Instant,tag=!InSafezone,tag=HasUUID] CmdData matches 30 as @e[type=marker,tag=ImpactMarker,tag=lightningstrike,tag=!HasUUID] run data modify entity @s CustomName set from storage craftycannoneers:temp CustomName
