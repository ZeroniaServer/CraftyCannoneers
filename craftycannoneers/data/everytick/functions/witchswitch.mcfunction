...
execute unless entity @s[tag=!bbarrel,tag=!bcannon] run summon villager ~ ~ ~ {NoAI:1b,Silent:1b,NoGravity:1b,Age:-2147483648,Tags:["newVillager"],PersistenceRequired:1b,DeathTime:17s,ActiveEffects:[{Id:6,Amplifier:10b,Duration:-1,ShowParticles:0b}],Passengers:[{id:"minecraft:marker",Tags:["newVillager"]}]}
team join NoName @e[type=villager,tag=newVillager,limit=1]
execute if entity @s[tag=bcannon,tag=!occupied] run ride @e[type=item_display,tag=BoatCannonDisplay,predicate=!game:mounted,limit=1,sort=nearest,distance=..2] mount @e[type=villager,tag=newVillager,limit=1]
#> TODO don't do this if we KNOW that the villager did not have the husk as a passenger
execute if entity @s[tag=bcannon,tag=occupied] run ride @e[type=husk,tag=BoatCannonDisplay,predicate=!game:mounted,limit=1,sort=nearest,distance=..2] mount @e[type=villager,tag=newVillager,limit=1]
execute if entity @s[tag=bcannon] run ride @e[type=villager,tag=newVillager,limit=1] mount @e[type=camel,tag=BoatCannonAnchor,sort=nearest,limit=1,distance=..2]
