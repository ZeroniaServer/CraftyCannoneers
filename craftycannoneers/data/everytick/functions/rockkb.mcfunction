...
execute rotated as @p[tag=thrower] as @e[type=llama,tag=myllama] positioned as @s run tp @s ~ ~ ~ ~ ~
execute unless entity @e[type=snowball,tag=pair] positioned as @p[team=!Spectator,tag=!thrower,distance=..3] run summon llama_spit ~ ~2.1 ~ {Tags:["myspit"]}
