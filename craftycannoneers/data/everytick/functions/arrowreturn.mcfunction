...
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] if score $arrowCount CmdData <= $arrowDiff CmdData run scoreboard players operation @s hasArrows += $arrowCount CmdData
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] if score $arrowCount CmdData <= $arrowDiff CmdData run tag @s remove arrowActionable

#> else if this stack is greater than amount needed, deduct arrowsToShoot and give back difference to player
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] run scoreboard players operation $arrowCount CmdData -= $arrowDiff CmdData
execute if entity @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] if score $arrowCount CmdData matches ..0 run kill @s
execute if entity @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] store result entity @s Item.Count int 1 run scoreboard players get $arrowCount CmdData
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] run loot give @s loot weapons:arrows_return
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] run scoreboard players operation @s hasArrows += $arrowDiff CmdData
execute as @a[team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] run tag @s remove arrowActionable
