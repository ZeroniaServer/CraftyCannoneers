...
execute as @a[team=!Lobby,team=!Spectator,predicate=game:matches_uuid] if score @s arrowsToShoot matches 1.. if score @s hasArrows < @s arrowsToShoot run tag @s add arrowActionable

execute if entity @a[team=!Lobby,team=!Spectator,predicate=game:matches_uuid,tag=arrowActionable] store result score $arrowCount CmdData run data get entity @s Item.Count
