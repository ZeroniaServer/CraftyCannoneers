...
execute if score @s BombDelay matches 1 as @a[tag=!Blasted,gamemode=!spectator,distance=..5] at @s run summon area_effect_cloud ~ ~155 ~ {Tags:["BlastAEC"],Particle:"block air",Age:-1,ReapplicationDelay:-1,WaitTime:0,Radius:0.1f,Duration:1,effects:[{id:"minecraft:levitation",amplifier:100,duration:2,show_particles:0b}]}
execute if score @s BombDelay matches 2 run tag @a[tag=!Blasted,gamemode=!spectator,distance=..5] add Blasted
execute if score $Wildlife CmdData matches 1 if score @s BombDelay matches 1 as @e[type=cat,tag=CrabVehicle,distance=..5] at @s run summon area_effect_cloud ~ ~155 ~ {Tags:["BlastAEC"],Particle:"block air",Age:-1,ReapplicationDelay:-1,WaitTime:0,Radius:0.1f,Duration:1,effects:[{id:"minecraft:levitation",amplifier:80,duration:2,show_particles:0b}]}
