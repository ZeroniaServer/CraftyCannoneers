...
#> Water kill
execute at @s[tag=!Kaboom,tag=!WaterKill] if block ~ ~ ~ water run scoreboard players reset @s CmdData
execute at @s[tag=!Kaboom] if block ~ ~ ~ water run tag @s add WaterKill
data merge entity @s[tag=WaterKill,scores={CmdData=1}] {Motion:[0.0,-0.02,0.0]}
execute at @s[tag=WaterKill,scores={CmdData=1}] run particle cloud ~ ~1 ~ 0 0 0 0.1 2 force
