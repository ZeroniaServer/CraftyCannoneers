...
execute at @s if entity @a[predicate=chests:key,distance=..7] run tp @e[type=villager,tag=LChestVillager] ~ -33 ~
execute at @s unless entity @a[predicate=chests:key,distance=..7] run tp @e[type=villager,tag=LChestVillager] @s